    ...
    <div className="space-y-4">
        ...
        {[
          ...
          <div key={label} className="bg-slate-50 rounded-lg p-3">
            ...
            <input
              ...
              className="w-full bg-transparent text-blue-600 font-mono font-bold text-lg border-b border-slate-200 outline-none"
            />
      ...
      <div className="space-y-2">
        {[
          ...
          <div key={s.num} className={`rounded-lg p-3 border ${s.highlight ? 'border-blue-500 bg-amber-50' : 'border-slate-200 bg-slate-50'}`}>
            <div className="flex items-start gap-3">
              <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${s.highlight ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-500'}`}>
                {s.num}
              </div>
              <div className="flex-1">
                <div className="text-sm text-slate-600 font-medium">{s.title}</div>
                <div className="text-xs font-mono mt-1">
                  <span className="text-slate-500">{s.formula}</span>
                  <br />
                  <span className="text-slate-500">{s.calc}</span>
                  <br />
                  <span className={s.highlight ? 'text-blue-600 font-bold text-sm' : 'text-slate-700 font-bold'}>{s.result}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className={`p-3 rounded-lg text-center ${actualN >= nd ? 'bg-emerald-50 border border-emerald-200' : 'bg-red-50 border border-red-200'}`}>
        <span className="text-sm text-slate-500">Actual factor of safety </span>
        <span className={`font-bold text-lg ${actualN >= nd ? 'text-emerald-400' : 'text-red-400'}`}>n = {actualN.toFixed(2)}</span>
        <span className="text-slate-500 text-sm"> {actualN >= nd ? `\u2265 n\u2090 = ${nd.toFixed(1)} \u2713` : `< n\u2090 = ${nd.toFixed(1)} \u2717`}</span>
      </div>
    </div>
  )
}
