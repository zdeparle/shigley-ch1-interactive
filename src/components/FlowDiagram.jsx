...
const phases = [
  ...
  { id: 4, label: 'Analysis &\nOptimization', x: 250, y: 270, color: '#2563eb', desc: 'Evaluating each scheme using mathematical models. Stress, deflection, cost analysis. Schemes that fail are revised or discarded.' },
    ...
    <div className="space-y-3">
      <svg viewBox="0 0 500 480" className="w-full" style={{ maxHeight: 420 }}>
        ...
        <path
          ...
          opacity="0.5"
        />
        <text x="440" y="240" fill="#2563eb" fontSize="11" textAnchor="middle" opacity="0.7"
          transform="rotate(90, 440, 240)">Iteration</text>
        <polygon points="390,400 396,390 384,390" fill="#2563eb" opacity="0.5" />
          ...
          return (
            <g key={p.id} onClick={() => setActive(isActive ? null : p.id)} className="cursor-pointer">
              <rect
                ...
                rx={10}
                fill={isActive ? p.color + '40' : '#f8fafc'}
                stroke={isActive ? p.color : '#e2e8f0'}
      ...
      {active && (
        <div className="bg-slate-50 rounded-lg p-3 border border-slate-200 transition-all">
          ...
          <p className="text-sm text-slate-500">{phases.find(p => p.id === active)?.desc}</p>
        </div>
      )}
      {!active && (
