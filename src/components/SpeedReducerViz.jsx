...
export default function SpeedReducerViz() {
  ...
  const sections = {
    ...
    gearbox: { label: 'Gearbox', color: '#f59e0b', desc: 'Contains gear train that reduces speed from 1750 to 82-88 rev/min. Must fit within 14×14×22 in. Efficiency >95%.' },
  ...
  return (
    <div className="space-y-3">
      <div className="bg-[#0e0e1e] rounded-xl p-4">
        <svg viewBox={`0 0 ${svgW} ${svgH}`} className="w-full">
          ...
          <g onClick={() => setActiveSection(activeSection === 'motor' ? null : 'motor')} className="cursor-pointer">
            <rect x={20} y={40} width={70} height={60} rx={8} fill={activeSection === 'motor' ? '#3b82f640' : '#141428'} stroke="#3b82f6" strokeWidth="2" />
          ...
          <g onClick={() => setActiveSection(activeSection === 'input' ? null : 'input')} className="cursor-pointer">
            <rect x={90} y={62} width={60} height={16} rx={3} fill={activeSection === 'input' ? '#8b5cf640' : '#252548'} stroke="#8b5cf6" strokeWidth="1.5" />
          ...
          <g onClick={() => setActiveSection(activeSection === 'gearbox' ? null : 'gearbox')} className="cursor-pointer">
            <rect x={150} y={20} width={180} height={110} rx={10} fill={activeSection === 'gearbox' ? '#f59e0b20' : '#141428'} stroke="#f59e0b" strokeWidth="2" />
            {/* Gear 1 (large) */}
            <circle cx={210} cy={55} r={25} fill="none" stroke="#f59e0b" strokeWidth="1.5" strokeDasharray="4 2" />
            <circle cx={210} cy={55} r={5} fill="#f59e0b" />
            {/* Gear 2 (small - input) */}
            ...
            {/* Gear 4 */}
            <circle cx={260} cy={45} r={12} fill="none" stroke="#f59e0b" strokeWidth="1.5" strokeDasharray="3 2" />
            <circle cx={260} cy={45} r={3} fill="#f59e0b" />
            <text x={240} y={145} fill="#f59e0b" fontSize="9" textAnchor="middle" fontWeight="600">Gearbox</text>
            <text x={240} y={17} fill="#f59e0b" fontSize="8" textAnchor="middle">14" × 14" × 22"</text>
          </g>

          {/* Output shaft */}
          <g onClick={() => setActiveSection(activeSection === 'output' ? null : 'output')} className="cursor-pointer">
            <rect x={330} y={77} width={60} height={16} rx={3} fill={activeSection === 'output' ? '#10b98140' : '#252548'} stroke="#10b981" strokeWidth="1.5" />
          ...
          <g onClick={() => setActiveSection(activeSection === 'application' ? null : 'application')} className="cursor-pointer">
            <rect x={390} y={50} width={80} height={60} rx={8} fill={activeSection === 'application' ? '#ef444440' : '#141428'} stroke="#ef4444" strokeWidth="2" />
      ...
      {/* Info panel */}
      {activeSection && (
        <div className="bg-[#0e0e1e] rounded-lg p-3 border border-[#252548]" style={{ borderColor: sections[activeSection].color }}>
          <div className="font-semibold text-sm mb-1" style={{ color: sections[activeSection].color }}>
            {sections[activeSection].label}
          </div>
          <p className="text-sm text-slate-400">{sections[activeSection].desc}</p>
        </div>
      )}
      {!activeSection && (
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-[#0e0e1e] rounded-lg p-2">
          ...
          <div className="font-mono font-bold text-[#f59e0b]">{(1750 / 85).toFixed(1)}:1</div>
        </div>
        <div className="bg-[#0e0e1e] rounded-lg p-2">
